...
export default function EditPage({ params }) {
  ...
  // 必要なプロパティが存在するか確認
const userId = signInUser?.id || signInUser?.uid;
  ...
  useEffect(() => {
    ...
    if (!params.id || !signInUser?.id) {
      ...
      try {
        ...
        const reviewResponse = await fetch(`/api/reviews?userId=${signInUser.id}&bookId=${params.id}`);
