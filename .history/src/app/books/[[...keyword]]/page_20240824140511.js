...
export default function BookResult({ params }) {
  ...
  ? books.slice((page - 1) * booksPerPage, page * booksPerPage)
  : [];
        ...
        <>
          ...
          {paginatedBooks.length > 0 ? (
            paginatedBooks.map((b, i) => {
              console.log("Rendering book:", b); // 追加
              return <LinkedBookDetails book={b} index={i + 1} key={b.id} />;
          })
          ) : (
            <p>書籍情報が見つかりませんでした。</p>
        )}
          ...
          <PaginationComponent
            page={page} // 現在のページ番号
            count={Math.ceil(books.length / booksPerPage) || 1} // 総ページ数
